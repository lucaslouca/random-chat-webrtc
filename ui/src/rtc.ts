...
export class WebRTCClient {
   ...
   */
  on<T extends RtcEventName>(eventName: T, handler: Handler<RtcEvent[T]>): () => void {
    const handlers = /** @type {Set<H>} */ this.#listeners.get(eventName) ?? new Set()
   ...
   */
  async openStream(options?: { cameraId?: string; audioDisabled?: boolean }): Promise<MediaStream> {
    ...
    this.#peerConnection?.connection.getSenders().map((sender) => {
      sender.replaceTrack(
        stream.getTracks().find((track) => track.kind === sender.track?.kind) ?? null
      )
    })
  ...
  #toggleTrack(enable: boolean, kind: string): void {
  ...
  /**
   * Send a signal to the connected peer that this user is typing
   */
  ...
  async #addIceCandidate(candidate: IceCandidate): Promise<void> {
    ...
    if (candidate.id !== peer.id) {
      console.warn(`Ignoring ICE candidate for unconnected peer ${candidate.id}`)
      return
    }
  ...
  async #handleServerMessage(
    message: RtcMessage | PeerMessage | ReadyMessage | HelloMessage
  ): Promise<void> {
      ...
      switch (message.type) {
        ...
        case 'peer':
          if (message.data.remove) {
            ...
            const event = this.#peers.has(message.data.id) ? 'peerupdated' : 'peeradded'
  ...
  #sendToPeer(message: ClientMessage): void {
    ...
    if (channel.readyState === 'open') {
      ...
      console.warn(`Not sending message to ${peerId} because channel isn't ready`)
    }
  }
}
